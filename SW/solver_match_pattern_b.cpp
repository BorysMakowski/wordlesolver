...
SolverMatchPatternB::SolverMatchPatternB(std::vector <std::string> _wordList) :Solver(_wordList)
	{
		prevResult = "";
		prevGuess = "";
	};
	...
	// HELPER FUNCTIONS
	*/
	void SolverMatchPatternB::showPattern()
	{
		for (int i = 0; i < 5; ++i)
		{
			std::cout << "[" << i << "]: ";
	...
	void SolverMatchPatternB::resetPattern()
	{
		for (int i = 0; i < 5; ++i)
		{
			pattern[i].clear();
	...
	void SolverMatchPatternB::removeFromPattern(char letter)
	{
		for (int i = 0; i < 5; ++i)
			pattern[i].erase(std::remove(pattern[i].begin(), pattern[i].end(), letter), pattern[i].end());
	}

	bool SolverMatchPatternB::matchesPattern(std::string word)
	{
	...
	bool SolverMatchPatternB::containsRequiredLetters(std::string word)
	{
